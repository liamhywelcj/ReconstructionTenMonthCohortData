function [gmSensitivity,concImage,tstatImage] = tstatsOxfordConditionVsBaseline(twLower,twUpper,conditionNumber,iChromo)
%This function produces the images for each condition relative to baseline
...
for iSubject = 1:59
    ...
    if iChromo == 1
        ...
        image_GM_Exl = hbo.gm(twLower:twUpper,:,conditionNumber);
        image_GM_Ctl = hbo.gm(1:20,:,conditionNumber);
    else
        load(dotimgFileName,'-mat','hbr')
        image_GM_Exl = hbr.gm(twLower:twUpper,:,conditionNumber);
        image_GM_Ctl = hbr.gm(1:20,:,conditionNumber);
    end
...
for i = 1:length(gmSurfaceMesh.node)
    [~,p,~,stats] = ttest2(image_GM_Exl_TOTAL(:,i),image_GM_Ctl_TOTAL(:,i));
    pVecContrast(i) = p;
    tVecContrast(i) = stats.tstat;
end

commonImage = mean(image_GM_Exl_TOTAL)';
...
SD = nirs.SD;
SD.MeasListAct = S;
[gmSensitivity, GMmask, ~, ~] = DOTHUB_MakeGMSensitivityMap(J_GM_wav1,J_GM_wav2,J_vol_wav1,J_vol_wav2,SD,0.01);
totalMask = GMmask;
...
concImage = commonImage.*totalMask'.*pMaskContrast;
